 ...
 * 이 한계를 임계값 (threshold) 이라 하며 theta (Θ) 로 나타냄
 ...
 y=\begin{cases}0\left( w_{1}x_{1}+w_{2}x_{2}\leq \theta \right) \\
 1\left( w_{1}x_{1}+w_{2}x_{2} >\theta \right) \end{cases}
 ...
 Note
 1. Perceptron 의 매개 변수를 정하는 것은 컴퓨터가 아니라 인간임
 2. 기계 학습은 이 매개 변수를 정하는 작업을 컴퓨터가 자동으로 하도록 함
 3.  (기계 학습일 때) 학습 : 적절한 매개 변수를 정하는 작업
 4.  (기계 학습일 때) 사람 : Perceptron 의 구조 (모델) 을 고민하고, 학습 데이터를 줌
...
/*:
 ...
 AND, NAND, OR 는 모두 같은 구조인 Perceptron 임
